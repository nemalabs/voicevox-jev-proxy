...
認証は無いので、`--host` で loopback 以外のアドレスにすると、そこに届く誰でも Jev へのリクエストを使い、VOICEVOX のすべての API を呼べる (起動時に警告を出す)。ほかのマシンから使うときは、そのマシンが送る Host を `--allow-host` で許可する。リクエストの本文は 1 MiB まで、同時に扱う接続は 32 本まで、クライアントの通信は 30 秒で打ち切る。
