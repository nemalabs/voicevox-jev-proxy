...
認証は無いので、`--host` で loopback 以外のアドレスにすると、そこに届く誰でも Jev へのリクエストを使い、VOICEVOX のすべての API を呼べる (起動時に警告を出す)。ほかのマシンから使うときは、そのマシンが送る Host を `--allow-host` で許可する。リクエストの本文は、`/audio_query` は 1 MiB まで、VOICEVOX に転送するもの (`/connect_waves` の WAV など) は 64 MiB まで。同時に扱う接続は 32 本まで、クライアントの通信は 30 秒で打ち切る。
...
- `--allow-host`: このサーバーを指す Host ヘッダー (`名前:ポート`) を足す。複数回指定できる。`localhost`、`127.0.0.1`、`[::1]` と `--host` のアドレスは最初から許可してある。それ以外の Host のリクエストは 403 を返す (DNS リバインディング対策)。
- `--max-text-length`: `/audio_query` で直す文の最大文字数。既定は 1000。超えると 413 を返す。
